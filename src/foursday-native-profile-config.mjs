...
export function foursdayCodexConfig({
  ...
  return [
    ...
    "enabled = false",
    "",
    "[permissions.foursday-classifier]",
    'description = "Foursday owner-intent classification without files, tools or network"',
    'extends = ":read-only"',
    "",
...
export async function buildFoursdayNativeProfileConfiguration({
  ...
  const environment = {
    ...
    FOURSDAY_TASK_LEDGER_FILE: join(stateRoot, "task-ledger.json"),
    ...
    FOURSDAY_PROFILE_RELEASE_FILE: join(layout.profileDirectory, "foursday-release.json"),
