...
export function ownerInterventionPrompt({ text, selfChat, taskActive, recentTaskText = "" }) {
  ...
  return [
    ...
    "task_correction: the owner replaces or contradicts the existing task goal, business rule, evidence standard, priority, or requested result.",
    "task_takeover: the owner takes over or cancels the whole task and asks the AI to stop working.",
    "resume_requested: the owner asks a paused or taken-over task to continue; it is not used while task_active=true.",
    "unrelated_owner_message: a normal new request, additive follow-up or question, acknowledgement, or conversation with no control intent.",
    "An additive follow-up during an active task stays unrelated_owner_message unless it explicitly replaces or contradicts the prior instruction.",
