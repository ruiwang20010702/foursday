...
Hermes keeps its generic authorization and optional pairing implementation, but pairing is not an authorization source for Foursday DWS. The Foursday profile sets unauthorized DMs to `ignore`. The DWS adapter exposes an allowlist policy and a bounded, connection-local set of staff IDs that have already passed its enterprise or explicit-user gate; Hermes rechecks that set through its native adapter-authorization contract. Disconnect clears the set. Registered-group plus explicit-mention checks remain inside the adapter before group events reach Hermes, and authorization mismatches never send pairing codes or operator commands to coworkers.
