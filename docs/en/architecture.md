...
The current source candidate is `v0.9.0-rc.5`; the latest GitHub Release remains `v0.8.0-rc.1`. Source validation, a GitHub release, technical deployment and production activation are separate states. The detailed product contract lives in the [PRD](../产品需求文档.md).
...
Binding selection first matches the current owner revision and send generation, then falls back to the newest valid binding; a stale unbound legacy record moves to recent history instead of appearing as AI-owned. Project discovery remains automatic and never requires the requester to create a folder. A Codex scope selection updates the same-generation Control projection immediately. Registry or exact-link access changes trigger a bounded, silent reconciliation Turn for unresolved tasks; Codex re-reads current evidence and corrects the project or blocker without asking the owner to clean up state, while the connector suppresses the maintenance reply in both Shadow and Active. The Turn preserves the original external request boundary—a linked document is evidence and cannot expand an access or reading request into delivery of the whole project. It cannot modify project files or external systems, may mark completion only when current evidence already proves the original requested outcome, and otherwise remains escalated with one real human action instead of falling back to acceptance. AI-owned means control responsibility remains with the agent, not that a Turn is currently running. The worksite shows the bounded requester label and channel, the Foursday/Codex executor, Thread location, a deterministic progress snapshot, semantic contract, bounded evidence, missing proof, and sanitized App Server activities such as read/search/edit/test—never raw reasoning, full commands, chat bodies, stable identity IDs, paths, or tool arguments. Missing contracts are reported as evidence-not-started instead of misleading zero counts. Pause, resume, communication takeover, and task takeover invoke the existing revision-fenced Control CLI; the worksite cannot enable sending, deploy, write gbrain, delete data, or expand permissions. A `codex://threads/<id>` link appears only when the Thread is resolvable in the current desktop data space; isolated Threads are explicitly described as absent from the main sidebar and expose only a copyable diagnostic ID. The companion validates a loopback service package-version handshake and replaces only an exact same-user Foursday listener, so an upgraded binary cannot silently reuse stale in-memory UI code. It never patches the Codex app or creates a second task state machine.
...
Codex also projects its semantic understanding into a private current-task contract: title, goal, deliverables, acceptance criteria, lifecycle, and a bounded Evidence Manifest. This is not a capability manifest or keyword classifier. Foursday derives the anonymous task identity, requester hash, project, owner revision, and send generation from the current context token. Stale generations are rejected. Codex may set `completed` for verified reversible work that does not require business sign-off; `waiting_acceptance` needs verified evidence and an actual acceptance decision, and the Agent still cannot set `accepted` for its own work.
