...
- 发送正文先转换为稳定钉钉纯文本；账本同时保存规范摘要与忽略Markdown/空白差异的指纹。服务端把反引号转换为粗体、折叠换行，把有序列表重新编号，或把本地Markdown链接的绝对路径移除、仅保留标题和行号时，两侧生成同一指纹并完成唯一回读；标题、链接行号、正文中的普通数字和普通正文仍参与指纹，不能把时间邻近但内容不同的人工消息误判为AI消息；
