...
任务锚点由消息聚合和Codex任务边界共同决定。只有同一时间bundle含两条以上消息且责任标签功能开启时，DWS Personal调用独立无工具Codex分类回合，返回固定的连续消息索引分组；每个索引必须恰好出现一次且保持顺序，低置信度、超时或协议漂移保守合为一个bundle。分组器只切分Hermes MessageEvent，不执行项目工具、不生成用户回复，也不成为第二Agent Loop。同一任务的新补充到达时，Connector先递增generation，再以CAS把自动reaction从旧`sourceMessageId`迁移到新锚点；不同任务分别进入Turn并维护各自最后消息锚点。单条消息包含多个任务时，一个reaction仍只能表示整条消息已被接纳，不能冒充子任务级状态。
