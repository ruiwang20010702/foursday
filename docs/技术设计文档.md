...
- 长任务不另建Agent Runtime。主Codex在实质工具前调用`foursday_set_execution_plan`，语义输出`instant|foreground|background`、步骤数、是否外部等待、是否需跨重启和自然接单文本；程序以确定性边界校正：显式耐久/等待或多步骤长任务直接background，未命中时由App Server活动监督器按真实耗时与工具活动升级。分类不可用默认foreground，不能阻止主Agent工作；
