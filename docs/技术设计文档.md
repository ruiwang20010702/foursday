...
伴侣只读取`http://127.0.0.1:9466/api/status|tasks`，服务不存在时启动`foursday dashboard --port 9466`子进程；请求超时1—3秒、10秒刷新、没有公网访问。鼠标进入宠物时只挥手一次，随后即使光标停留也恢复静止；点击会跳跃并扩展同一Panel，左右拖动会播放相应方向动画并移动窗口，任务首次进入`accepted`或任务列表清空时会短暂庆祝；异常状态不会锁死这些互动。抽屉展示任务标题、目标、生命周期和最多三条验收条件；不显示聊天原文、稳定身份、任务哈希、工作区路径或凭据。关闭伴侣会停止自己启动的dashboard，不停止Gateway。构建脚本默认只预览，`npm run pet:build -- --apply`在`.runtime`生成ad-hoc签名候选，不自动安装到`~/Applications`或登录项。
