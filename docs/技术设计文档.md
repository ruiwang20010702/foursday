...
Runtime MCP与Owner Control MCP保持两个安全域，但共享同一可靠性口径。Runtime MCP的五个工具必须显式声明`readOnlyHint`、`destructiveHint`、`idempotentHint`与`openWorldHint`；缺失注解会让Codex 0.145把只读调用标记为需要批准，在无交互客户端中表现为`user cancelled MCP tool call`，code-mode-host随后等待父进程响应。Profile因此设置5秒启动和10秒单工具上限，影子客户端对未知Server Request立即失败。项目记忆客户端在单个MCP进程内复用已验证的`default + read-only` OAuth会话；401/403仍由客户端刷新，Profile更新/进程重启会清除缓存，不跨身份或配置复用。
...
- 出站使用8秒基础稳定窗口，并把本轮消息发现延迟计入补偿，最长20秒；新碎片到达时递增`sendGeneration`，旧结果不得进入发送账本。队列合并必须保留全部可见文本、聚合源消息ID，但只保留最终事件的工作上下文token和channel prompt；由于Hermes可能在同一个底层处理调用内继续队列Turn，Adapter不能只依赖任务`ContextVar`，而是维护每个会话的最新源消息ID与版本。仅当最终`reply_to`精确等于该最新消息ID、会话一致且版本单调不降时，才采用最新`ownerRevision/sendGeneration`；随后Sidecar仍重验全局/任务状态、探针、generation和发送账本。空锚点、旧锚点、错会话、后续新消息或接管后的回复不能借用最新版本；
- 检查点权限为 `600`，重启重叠抓取但不重复触发；
- 发送必须得到服务器 ID 或唯一正文回读；未知结果禁止重试；
- 发送正文先转换为稳定钉钉纯文本；账本同时保存规范摘要与忽略Markdown/空白差异的指纹。服务端把反引号转换为粗体或折叠换行时，仍可完成唯一回读并阻止AI消息回流；
...
`npm run runtime:verify-mcp -- --iterations=20`通过Codex App Server的`mcpServer/tool/call`稳定接口，对状态、附件列举、项目记忆读取和附件暂存各执行20次；记忆候选因会写PostgreSQL，仅在隔离数据库测试。验收要求80次成功、失败0、各工具P95小于2秒，并回收App Server/MCP子进程和暂存文件，不改变Codex信任目录，不发送消息、不写gbrain、不运行计划、不部署。
...
- Foursday Runtime MCP：只暴露实时Profile状态、注册项目gbrain精确只读、证据绑定的记忆候选，以及当前消息附件的列举/暂存工具；不暴露DWS发送、数据库、Git或任意shell。附件宿主路径只存在于600权限短期上下文，模型先列举匿名索引，再暂存到`.foursday-inbox/`；暂存文件标记`commitAllowed=false`。direct/group/cron都只能读取注册slug；仅私聊可提交稳定事实候选；私聊/群聊可读取当前附件，Cron拒绝附件。工具权限注解、5秒启动、10秒调用上限和真实App Server基准是发布门禁。
...
- 若后一段在首包已进入Codex后才到达，Hermes不执行active-turn redirect，而是以`queue`模式合并同发送者后续文本；合并结果删除旧工作上下文标记并仅附加最终事件token，防止代理按较旧generation拒绝工作区。首Turn沿用旧generation并被Sidecar抑制；队列最终回复只有在回复锚点命中当前最新源消息时才能采用最终事件版本。影子`reply_attempt`只记录脱敏的`ownerRevision/sendGeneration/latestReplyAnchor/generationRebound`及哈希，不记录消息ID或正文，使生产回读可以区分“锚点是否最新”“版本是否发生重绑”以及最终Sidecar结果。
