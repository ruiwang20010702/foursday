...
- 生产协议启动时Sidecar只声明transport ready，不预先消费历史消息；Python Bridge先释放有界事件缓冲并完成Gateway Adapter注册，再显式请求`reconcile`。只有此时Sidecar才读取、去重并把历史消息交给已注册回调，避免startup reconcile先把消息记为已见、Adapter却没有形成Agent入站证据。测试内默认启动仍可直接reconcile，便于独立验证；
