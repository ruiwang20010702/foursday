...
控制层采用端口与适配器方式渐进拆分，不重写Agent Loop，也不改变现有DWS、Hermes、Codex、gbrain和发送协议。`hermes-dws-sidecar.mjs`只作为组合根，最终仅负责启动、依赖注入和协议路由；业务状态变化必须进入对应协调器，禁止模块直接修改其他边界的内部状态。

| 边界 | 唯一职责 | 权威状态 |
|---|---|---|
| Runtime State Store | 旧状态规范化、容量与原子持久化 | 连接器临时状态文件 |
| Message Ingress | 多目标读取、历史补抓、排序与Checkpoint推进 | 连接器Checkpoint |
| Enterprise Identity | 当前企业身份验证、隔离队列和有界重试 | 连接器临时状态 |
| Task Coordinator | 消息组、任务generation、任务边界与Agent事件 | 当前Control generation；不保存Thread正文 |
| Thread Binding | Codex Thread、workspace、principal和权限版本 | 独立Thread Binding账本 |
| Responsibility Control | “好的”标签、reaction Watcher与表情接管 | reaction账本；接管结果提交Control Store |
| Owner Intervention | 文本介入语义、保守降级和revision事件 | Control Store |
| Delivery Coordinator | 安静窗、发送、回读、幂等和unknown | Send Ledger |
| Durable Execution | 长任务确认、队列、租约、完成与重启恢复 | Task Ledger execution |
| Checkpoint Coordinator | 检查代次、健康和唤醒合并 | 连接器Checkpoint |

当前控制层已经完成端口拆分：Runtime State Store独占默认值、旧文件规范化、容量限制、串行持久化与600权限原子发布；Checkpoint/Event Wake独占检查串行化、唤醒优先级、去抖、fallback计时与Stream健康投影；Enterprise Identity独占隔离队列、逐身份容量、退避、过期、拒绝与恢复；Durable Execution独占长任务确认、队列、租约和重启恢复；Responsibility Control独占reaction账本、Watcher、Owner稳定身份、标签迁移和事件接管；Owner Intervention独占有界任务上下文、语义分类、保守降级和介入事件；Delivery Control独占人工回复探针、安静窗口、延迟候选、Send Ledger、幂等键、发送、回读及unknown停发；Task Coordinator独占任务generation、接管后新任务重开、附件下载和Agent事件；Message Ingress独占多目标读取、历史切片、消息排序、Checkpoint推进和Control事件投影。`hermes-dws-sidecar.mjs`现只装配这些端口、注册文件/Event Watcher，并转发Hermes协议动作；它不再实现上述状态机或直接修改模块内部账本。

重构由五条零生产副作用黄金路径保护，统一通过`npm run check:golden`执行：

1. 普通消息进入并取得唯一服务器回读回复；
2. 六秒碎片使旧generation回复失效；
3. 负责人表情完成沟通接管且不再生成AI文本；
4. 长任务完成确认、排队、租约和终态；
5. 重启只恢复当前有效代次，不复活过期工作。

黄金路径之外仍必须执行完整JavaScript、Python、安装复用和安全回归。任何输出、权限、发送次数、状态格式或恢复语义变化都属于行为变更，不能以“重构”等名义合入。
...
sequenceDiagram
    ...
    X->>W: 最终回读
    X->>L: Evidence Manifest / waiting_acceptance或升级
    X-->>H: 自然回复与证据
    H-->>D: 持久化与投递
    D-->>U: DWS 精确发送
```
...
权限类来源读取复用`foursday_read_project_source`，不新增权限写工具。可选`accessRequired=read|download|edit|manage`触发第二条只读DWS `doc +inspect --include-permissions`调用；宿主要求`complete=true / hasMore=false / truncated=false`，以`SHA-256(permission.id)`与工作上下文中的`sourcePrincipalHash`比较，只返回`matched/roles/hasRequiredAccess`。成员ID、姓名、企业ID和完整ACL不进入Codex上下文。读取失败沿用`project_source_host_busy|host_unavailable|read_failed`，模型必须写“无法核验”而不是“未授权”。

Gateway启动前运行Codex项目增量同步，并通过`fs.watch`监听Codex状态文件所在目录以兼容原子替换；同名文件变化后1秒防抖、跨进程锁串行同步，无需重启Gateway。同步读取用户私有`.codex-global-state.json`中的`local-projects/rootPaths`；该Codex文件允许其他本机账号只读，但必须由当前用户拥有、不可被其他账号写且不得经过符号链接。同步复用项目发现的规范路径、敏感目录、宽泛目录、符号链接、Git remote和父子关系校验，只新增或保留已有workspace/scope；并在5秒内只读列举个人gbrain项目，用唯一精确或一字符名称变体补充页面关联，失败时仍完成本地项目接入。目标注册表使用600权限原子替换，先写同目录恢复副本，精确回读与`normalizeWorkScopeRegistry`通过才报告成功；同步失败记录固定诊断码但不阻断旧注册表启动。保存到Codex侧边栏是授权来源，任意磁盘扫描、未保存目录和gbrain相关页均不能扩大文件权限；完整gbrain标题不会复制成企业可见项目别名。

Control Service读取同一`routes.json`并按`sourceSessionHash=taskId`连接任务。若路由主范围存在且不同于当前fallback，`projectGroupId/projectGroupName`立即采用已选范围，另返回`routeSelection.pendingWorkspaceSwitch=true`；任务执行CWD仍在下一Turn由App Server重新绑定，UI不得冒充当前Turn已切换。

任务语义不进入Control Store。Control Store继续只保存会话级暂停/接管、`ownerRevision/sendGeneration`、当前项目投影和待消费控制事件；`foursday-task-ledger.mjs`另存当前任务合同的私有投影，权限600、最多1000项，不保存聊天原文、人员身份或凭据。Codex通过`foursday_update_task_contract`提交标题、目标、交付物、验收条件、生命周期与有界证据摘要；MCP从短期contextToken派生匿名`sourceSessionHash`、请求人哈希、项目、owner revision和send generation，模型不能指定这些身份字段。Codex选择新主项目后，route state与同generation的Control项目投影同时更新；后续合同读取该投影，不再停留在`shared_link`。旧generation不能覆盖新合同。`completed`表示可恢复工作已完成并有证据，不等于业务验收；`waiting_acceptance`只用于确实需要请求人/负责人决定的任务且至少包含一条verified证据；Agent输入枚举不含`accepted`，正式验收仍由稳定请求人/负责人反馈写入。

注册表或精确链接访问状态变化后，`foursday-task-reconciliation.mjs`只筛选仍为`escalated`或`waiting_acceptance`、同企业身份已验证、具有私有精确source binding且任务generation仍有效的任务，按15/30/60/180分钟有界重试触发同一Codex Thread的静默复核。复核必须保持原始外部请求边界，链接只提供证据，不能把权限或阅读请求扩大成整个项目交付。Codex重新读取source、gbrain与项目图，修正主/相关项目并更新合同；维护Turn不得修改项目文件或外部系统，只有当前证据本身已证明原请求完成时才能写`completed`，否则保持`escalated`并指出真实人类动作，不以`waiting_acceptance`掩盖不确定性。Connector通过turn-local标记硬性抑制维护回复，Shadow与Active均不会因此对外发送。source node与请求人哈希只保存在权限600的运行状态中，不进入Git、日志或用户界面。

桌宠与9466服务之间使用`foursday-control-site-meta/v1`版本握手。版本不一致或旧服务缺少meta时，桌宠使用`dashboard --replace`启动当前CLI；替换逻辑只允许终止同一macOS用户、精确命令路径与端口均匹配的Foursday只读服务，端口属于其他进程时失败关闭，不做宽泛清理。

这里的“智能”与“确定性”是两层：Codex根据完整消息组、历史Thread、主/相关范围、个人gbrain和工具证据判断任务、目标、验收、返工与升级；程序只验证身份、路径、版本、幂等、状态合法性、最小证据与外部风险。关键词候选只能用于紧急停止等毫秒级安全快路，不能决定项目、岗位合同、完成结论或自然回复。
...
- 全局暂停只改变global状态并失效在途generation，不覆盖每个任务自身的`active|paused|taken_over`；恢复全局运行也不得复活已接管任务。全局暂停或单任务暂停期间，未读消息保留到精确恢复，不推进对应检查点；`taken_over`只约束被接管的任务generation，不能永久封锁“会话+联系人”。新消息先由无工具Codex结合当前文本、最近24小时最多8条有界DWS会话上下文和接管时间判断`new_task|same_task`：明确旧任务续接继续静默；新任务以ControlStore精确revision CAS重开active generation并进入Agent Loop。分类不可用时优先恢复新任务，生产、付款、删除等高风险出口仍独立门禁。任何分支都不得阻断企业共享检查点；
- 30秒fallback下正常新鲜窗为60秒、有界忙碌硬期限为120秒；其他fallback按有界公式派生，最长15分钟，并容忍最多5秒文件时间戳漂移。该期限只适用于已经持久化的运行代次，不得把普通旧成功无限续命；
- 生产协议启动时Sidecar只声明transport ready，不预先消费历史消息；Python Bridge先释放有界事件缓冲并完成Gateway Adapter注册，再显式请求`reconcile`。只有此时Sidecar才读取、去重并把历史消息交给已注册回调，避免startup reconcile先把消息记为已见、Adapter却没有形成Agent入站证据。若检查点与当前时间相差超过DWS单次一小时上限，Sidecar按连续一小时片段分别读取并以稳定消息ID去重；不扩大单次DWS范围，任一片失败则整次检查失败且不推进该目标检查点。测试内默认启动仍可直接reconcile，便于独立验证；
...
- 长任务不另建Agent Runtime。主Codex在实质工具前调用`foursday_set_execution_plan`，语义输出`instant|foreground|background`、步骤数、是否外部等待、是否需跨重启和自然接单文本；程序以确定性边界校正：显式耐久/等待或多步骤长任务直接background，未命中时由App Server活动监督器按真实耗时与工具活动升级。15秒内完成的instant不发送接单套话；运行超过15秒且已有实质活动的foreground进入一次普通接单，但仍在当前Turn继续；需要外部等待、跨重启或多步骤耐久执行才进入background。分类不可用默认foreground，不能阻止主Agent工作；
- 任务账本新增generation绑定的execution记录：`foreground|ack_pending|acknowledged|queued|running|blocked|completed|failed|cancelled`、executionId、计划摘要、活动计数、租约、尝试次数、接单状态与固定错误码。接单正文允许作为有界AI生成交付物保存，但继续禁止聊天原文、原始reasoning、完整命令、绝对路径、人员身份和凭据；
- DWS Personal的处理Hook在Turn执行期间轮询当前execution。background或运行时升级后只发送一次`interim_ack`；该发送复用Sidecar人工探针、安静窗、Control/generation和发送账本，但不清除责任reaction。首Turn随后返回`NO_REPLY`，`on_processing_complete`通过Bridge把execution从`acknowledged`推进到`queued`并发出`internalBackground=true`记录；Adapter跳过外部消息去重但重新生成15分钟工作上下文，调用Hermes同一Session队列，Codex代理按原Thread绑定恢复；
- 内部续跑事件不是用户消息，不推进DWS入站检查点，不重新贴标签，不参与普通责任分组。它携带原消息回复锚点、executionId和当前代次；后台Turn的最终回复带`deliveryKind=background_final`，Sidecar的幂等键对该类型只使用`conversationId/executionId/deliveryKind/generation`，忽略重跑产生的措辞变化。完成回执后execution终态化；unknown继续全局停发；
- Sidecar启动对账`queued`和租约过期的`running`记录，仅在Control仍active且ownerRevision/sendGeneration精确一致时重发内部续跑事件。人工回复、沟通接管、任务接管、纠正、新消息generation、全局暂停或Thread/项目漂移均使旧事件与旧最终回复失败关闭；
- 出站使用8秒基础稳定窗口，并把本轮消息发现延迟计入补偿，最长20秒；新碎片到达时递增`sendGeneration`，旧结果不得进入发送账本。队列合并必须保留全部可见文本、聚合源消息ID，但只保留最终事件的工作上下文token和channel prompt。实时消息仍按源时间间隔使用短bundle；同一次`startup/history_reconcile`中的消息以相同`detectedAt`作为一次只读回放批次，不因历史源时间间隔提前flush，而先交给无工具Codex连续分组，同任务只形成一个MessageEvent、独立任务仍拆分。每个最终任务组再调用独立无工具Codex判定`action_required|no_text_reply`，结果写入15分钟私有工作上下文；高置信`action_required`约束主Codex不得静默，但不生成回复、不调用项目工具、不取代主Agent推理。分类不可用时标为`availability_fallback`并由主Agent完整检查。Foursday包装固定的`GatewayRunner._handle_message`：主处理函数返回时，把当前会话最新版本冻结为“已消费快照”；普通测试或其他handler只能冻结本事件版本，不能假定消费了后来消息。Base最终发送携带`notify=true`并引用本次处理根消息；只有根锚点匹配、已消费快照仍等于会话当前最新版本且版本单调不降时，才采用快照中的`ownerRevision/sendGeneration`。若处理返回后新消息抢先到达，当前最新版本与快照不再相等，旧回复仍被Sidecar拒绝。随后Sidecar继续重验全局/任务状态、探针、generation和发送账本；
...
Codex原生pet的本机扩展面只包含名称、描述、spritesheet与动画版本；当前渲染组件仅按Codex自身状态播放动画并在悬停时触发`jumping`，没有插件级点击回调、任务数据源或打开插件面板的公开契约。Foursday不得修改`ChatGPT.app/app.asar`或依赖未公开内部符号。

`distribution/pet/macos/FoursdayPet.swift`因此提供一个可选、独立的macOS伴侣：AppKit无边框浮动Panel承载SwiftUI，`LSUIElement=true`不占Dock；从`~/.codex/pets/<id>`读取现有8×9或8×11 spritesheet，默认跟随Codex `config.toml`中的`selected-avatar-id`，`FOURSDAY_PET_ID`仅作为显式运行覆盖，缺失时显示Foursday品牌图形。九个标准动画行全部接入：`idle=0 / running-right=1 / running-left=2 / waving=3 / jumping=4 / failed=5 / waiting=6 / working=7 / review=8`。播放器先按Codex契约限制每行上限，再在加载时扫描每格Alpha像素，只播放真正有内容的列：空闲固定第1个有效帧，标准上限为左右奔跑与异常8帧、挥手4帧、跳跃5帧、等待/工作/验收6帧。因而任何合规8×9或8×11桌宠可以直接替换，第三方图集的透明占位格也会自动跳过。其中空闲、工作、等待、验收和异常来自运行健康与任务账本；Shadow、发送关闭、Gateway主动停止和由此产生的`stale`属于空闲而非异常，只有状态服务不可达、发送被阻断、模式不一致、明确失败状态或失败任务才进入异常。已接管任务不再产生待处理红点。悬停、点击、左右拖动和完成庆祝是优先级更高但会自动消退的表现状态，不回写任务账本，不改变Agent判断。

伴侣只读取`http://127.0.0.1:9466/api/status|tasks|schedules|memory|evidence`，这些端点全部来自同一个`FoursdayControlService`；服务不存在时启动`foursday dashboard --port 9466`子进程。请求超时1—3秒、10秒刷新、没有公网访问。鼠标进入宠物时只挥手一次，随后即使光标停留也恢复静止；点击会跳跃并把同一Panel向左扩展成工作现场。拖动手势只决定方向动画，窗口位置由AppKit在按下时记录`NSEvent.mouseLocation + panel.frame.origin`，后续按屏幕绝对鼠标差值计算并限制在当前显示器可见区域，禁止使用随窗口移动而变化的SwiftUI累计translation作为窗口原点，避免反馈漂移。异常状态不会锁死互动。关闭伴侣会停止自己启动的dashboard，不停止Gateway。构建脚本默认只预览，`npm run pet:build -- --apply`在`.runtime`生成ad-hoc签名候选，不自动安装到`~/Applications`或登录项。

Panel采用两层信息架构。默认`worksite`层保留项目—任务树、需要我/AI负责/最近完成、任务详情和revision控制；`diagnostics`层通过面板内“设置与系统诊断”进入，展示经过产品翻译的钉钉连接、Codex工作环境、消息同步、个人记忆、安装版本、运行证据和主动工作。`DisclosureGroup`内才展示mode/sendEnabled、Checkpoint代次、Event Wake、reaction watcher、固定绑定、可发现项目、完整提交与固定错误码。`PetModel`是这五个只读投影的唯一观察模型；页面切换和Disclosure展开只使用`@State`/`@Observable`本机状态，不写Control Store。

诊断采用单一建议规则：`sendBlocked`优先建议检查钉钉并保持暂停；Gateway未运行建议在Codex中检查Foursday状态；gbrain发现失败说明当前任务可继续并等待自动恢复；其他降级统一建议在Codex中检查Foursday状态。面板不直接提供激活、部署、写gbrain、清空记录、重启或权限扩张按钮。浏览器HTML仍复用相同GET端点和CSP/Host门禁，只作为桌宠依赖的loopback服务与隐藏应急/非macOS页面，不再形成另一套产品导航。

工作现场不复制稳定身份ID或聊天正文。DWS完成稳定身份校验后，私有Control Store只保存不参与鉴权和路由的有界联系人显示名与`dingtalk_direct|dingtalk_group`渠道类型；无显示名时保持空值，不用userId冒充联系人。Control服务将任务投影为`needs_me|working|recent`三类，界面顶部只汇总“需要我／AI负责／最近处理”的数量；`working`只表示Control所有权仍为active，不冒充实时Turn状态。任务列表再跨状态按稳定`projectId`聚合成唯一项目节点，SwiftUI侧栏内联展开项目及全部任务，每条任务自行显示所属状态；没有下拉选择器，同一项目在整个侧栏只出现一次。

项目节点默认展开，`collapsedProjectIds`只存在于桌宠本机观察状态；点击项目标题切换折叠，不调用Control服务，也不改变任务、Thread或Gateway。任务标题优先级为`taskContract.title → generation精确匹配的summaryTitle → 历史时间回退`。`scripts/回填历史任务摘要.mjs`默认只预览；显式`--apply`时按精确`codexThreadId`读取隔离Session中最多8条目标时间附近的`current_user_request`，交给禁用工具、Web、Memory和子Agent的有界Codex生成标题。任务账本只保存标题、`ownerRevision`、`sendGeneration`和更新时间；代次不一致的摘要不展示。Session不唯一、秘密检测、低置信或解析失败全部跳过。

Thread绑定读取不能把`sourceSessionHash`相同的文件直接交给`new Map()`按遍历顺序覆盖。对每个Control任务，选择器依次执行：① `ownerRevision`与`sendGeneration`同时精确匹配；② 仅当没有精确项时，按`sendGeneration → ownerRevision → updatedAt`选择最新有效绑定；③ 决胜条件使用稳定Thread ID，保证结果与文件名和目录遍历顺序无关。选中的绑定提供`projectId`和`codexThreadId`，项目注册表再投影`projectName`。

没有项目、任务合同和Thread绑定的记录分两类：距`lastInboundAt|updatedAt`不超过10分钟的active任务为`routing`，仍在AI负责区显示“正在识别项目”；超过窗口的旧记录为`legacy_unassigned`，强制进入最近处理下的“未归档历史”，不产生红点、不计入AI负责。该状态只说明历史路由证据不完整，不表示请求人需要创建项目名或文件夹。项目仍由Codex结合消息、会话、gbrain与本地注册表自动发现；程序不得为未知项目臆造归属。

响应还包含请求人显示投影、`Foursday · Codex`执行者、真实Thread位置、任务合同、进度快照、Evidence Manifest计数、`missing`证据摘要、最多20条脱敏活动以及当前Control revision。没有任务合同时Evidence计数为未开始统计，不能用三个0冒充进度；已有Thread但尚无活动时明确显示“会话已建立，尚无可展示活动”。App Server通知先经过纯函数分类：reasoning只记“正在分析”，命令只根据类型映射为读取/搜索/修改/测试/验证，文件只保留basename，MCP只记工具类别；原始reasoning、完整命令、工具参数、绝对路径和模型回复不写任务账本。活动使用事件摘要幂等去重，单任务有界保留。

原生面板的控制动作不新增HTTP写接口。App通过已安装的`foursday control`CLI执行`pause-task|resume-task|communication-takeover|takeover-task`，每次携带刚读取的`revision/taskId`，CLI再调用同一`FoursdayControlService.apply`。revision冲突只刷新界面并要求用户重新操作；不自动重放。该入口不能开启发送、部署、gbrain写入、计划、删除或权限扩张。

Codex Thread可见性采用双模式。`isolated`保持现有专属`CODEX_HOME`与独立登录，安全边界最强，但主Codex侧边栏不能解析其Thread；此时面板明确说明隔离位置并允许复制非秘密Thread编号用于诊断，不能提供无效深链或暗示会出现在主侧边栏。`desktop-visible`是个人模式下的显式选择：App Server运行在主Codex数据空间并加载独立Foursday config profile，Thread必须为非临时、绑定真实Codex Project并设置标题，完成后使用`codex://threads/<id>`打开。进入该模式前必须验证未授权MCP不会被继承，workspace sandbox和Foursday代理仍是强制门禁；没有完成此负向验收前默认保持`isolated`。
...
| 6.5 Codex Loop | `foursday-codex-proxy.mjs`、`foursday-task-ledger.mjs`、Profile、project-work Skill | Codex唯一工具循环；语义生成任务合同；安全resume/fork；Web/Image/MCP/Skills/Memory/Subagent按Profile开放；模型不得自证accepted；调用方不能覆盖策略 | Proxy、任务账本、真实App Server与能力负向测试 |
...
| 6.10 Agent操作入口 | `foursday-control-*`、Codex/Claude插件、可选状态页、`distribution/pet/macos/` | 同一Control服务与任务账本；revision CAS；Codex pet素材复用；桌面伴侣只读loopback；发现失败不拖垮Gateway；只读页无POST；不修改Codex App；不复用旧9465 | Control Store/Service/MCP/Site、task ledger、macOS pet构建与真实点击QA、个人记忆分页、DWS代次与插件校验 |
...
本人自聊中的普通任务文本不调用语义分类器；只有宽松候选命中才进入分类，但“继续上一条”究竟是普通追问还是恢复任务仍由Codex结合任务状态和最近上下文判断，正则不能直接决定。Bridge只向分类子进程传递当前有界消息、最近任务摘要、独立Codex Home、固定工作区/注册表以及启动Codex包装器所必需的Node可执行路径；不传业务密钥、其他DWS数据或普通项目工具。第三方会话中的负责人真实发言一律先冻结，Codex即使判断为`unrelated_owner_message`也按`communication_takeover`处理，因为负责人已经实际代表本人对外发言。语义服务不可用时同样保守接管，不允许把可能的控制消息重新投入普通Agent Loop。分类证据只记录`codex/emergency/conservative_fallback`和高/中/低置信度桶，不记录负责人正文或隐藏推理。

### 14.7 消息责任reaction与负责人reaction接管

Foursday不重新实现通用reaction运行时。原生Hermes已经提供`on_processing_start/on_processing_complete`生命周期和钉钉机器人处理状态reaction；DWS Personal在同一Hook上增加个人账号适配、任务锚点和Foursday发送栅栏。OpenClaw的`ackReaction/ackReactionScope/statusReactions`只作为跨平台配置语义参考，不引入第二运行时。

“好的”是消息责任状态的钉钉投影，不是任务权威。任务权威仍由Foursday Control、Codex Thread绑定、`ownerRevision/sendGeneration`和发送账本组成：

```mermaid
stateDiagram-v2
    [*] --> admitted: 身份与会话门禁通过
    admitted --> covered: 轻量责任判断为AI承担
    covered --> working: 需要完整Agent Loop
    covered --> no_reply: 已判断且无需回复
    working --> waiting: 等待业务信息
    waiting --> working: 请求人补充
    working --> delivered: 唯一发送回执成功
    covered --> owner_needed: 需要负责人行动
    covered --> taken_over: 负责人消息或reaction接管
    working --> failed: 永久失败/取消/撤回
    delivered --> [*]: 取消“好的”
    no_reply --> [*]: 保留“好的”作为已处理证据
    owner_needed --> [*]: 取消或替换为等待标记
    taken_over --> [*]: 取消并废弃旧发送版本
    failed --> [*]: 取消或替换为失败标记
```

任务锚点由消息聚合和Codex任务边界共同决定。实时bundle或同一次历史reconcile批次含两条以上消息时，DWS Personal调用独立无工具Codex分类回合，返回固定的连续消息索引分组；该分组不依赖可见reaction功能是否开启。每个索引必须恰好出现一次且保持顺序，低置信度、超时或协议漂移保守合为一个bundle。历史reconcile只在`wakeSource=startup`且`detectedAt`精确相同时跨越实时源时间上限，避免回放把同一长任务拆成多个Hermes Turn；日常实时消息不使用该放宽。分组器只切分Hermes MessageEvent，不执行项目工具、不生成用户回复，也不成为第二Agent Loop。每个任务组的响应责任分类同样是无工具、固定JSON枚举和有界正文；它只阻止可执行请求误用`NO_REPLY`，不能固定回复内容或跳过主Codex工作。同一任务的新补充到达时，Connector先递增generation，再以CAS把自动reaction从旧`sourceMessageId`迁移到新锚点；不同任务分别进入Turn并维护各自最后消息锚点。单条消息包含多个任务时，一个reaction仍只能表示整条消息已被接纳，不能冒充子任务级状态。

自动reaction写入权限`600`的私有有界账本，保存平台移除所需的会话/消息ID、reaction名称、源消息集合、generation、状态和时间，不保存消息正文、人员展示名或凭据；Control与状态页只投影哈希、计数和固定错误码。添加、迁移、取消和重启对账均幂等；reaction失败不终止后台工作，但不得被解释为用户已看见标签。可见回复只有在唯一服务器回执成功后才能清除责任标签；`send_unknown`保留标签与发送阻断。无需回复的成功Turn进入`handled_no_reply`并保留标签；新generation只迁移仍在处理的标签，不删除该静默终态。

负责人reaction是独立入站事件，不从消息当前reaction快照推断。DWS契约返回：`messageId`、`conversationId`、`action=added|removed`、规范emoji、稳定`operatorOpenDingTalkId`、服务端`occurredAt`和`eventId`。展示名、reaction计数或无时间快照不产生控制权。负责人`added`事件命中活跃任务锚点后，Connector先冻结旧generation，再确定性写入`communication_takeover`；reaction本身就是负责人已完成的对外回复，不调用Codex、不生成AI文本。负责人手动移除Foursday当前“好的”同样接管，其他移除只审计且不恢复发言。Foursday/Hermes自己的自动reaction通过写前intent和回执账本排除；其他成员、错会话、错消息和历史reaction不触发接管。

reaction事件可能先于DWS消息正文投影到达。已验证为负责人的`added`事件若暂时找不到同消息锚点，进入最多5分钟、128条的私有待关联队列；正文以稳定`messageId`到达后先形成最新锚点，再按原服务端时间重放接管。队列不保存正文，重启可恢复，过期后只留去重ID。Hermes忙碌队列合并补充消息时，`MessageEvent.message_id`必须同步迁移到最后一条源消息；Shadow责任记录也参与锚点迁移，防止“标签留在上一句、负责人点了下一句却未接管”。

`communication_takeover`写入后，后到的Hermes处理开始Hook必须同时校验最新control revision和`takeoverReported`，禁止补建责任标签；重启时清理已接管会话的残留Shadow/不可用责任记录。人工接管是责任终态，不允许异步Hook把它逆转。

DWS v1.0.59不支持`all-direct + reaction`全量订阅，事后`list-emotion-replies`又只返回显示名，不能作为负责人身份权威。Foursday改用消息Stream中的稳定`conversation_id + sender_open_dingtalk_id`在正文投影前预热该单聊reaction Stream；重启只恢复已有稳定OpenID的活跃会话，旧状态缺OpenID时跳过而不猜测。这样不扩大消息权限，也覆盖“首条消息后立即点表情”的时间窗。

当前DWS历史查询只能回读reaction名称和展示人，不能用于控制；DWS v1.0.59 reaction事件流另行提供稳定`operator_open_dingtalk_id`、目标消息、操作类型和时间，因此实现只消费事件流，历史快照仅用于界面回读。上线顺序固定为：先验证负责人reaction事件和自回流排除，再开放自动“好的”标签。
...
`foursday dashboard`是按需启动的127.0.0.1只读loopback服务，默认9466，只暴露GET投影，不提供控制写接口，也不保存状态。macOS普通用户由桌宠工作现场消费这些端点；浏览器HTML仅作隐藏应急/非macOS入口，不进入默认导航。旧9465管理台与旧任务/审批状态机不进入新架构。时间返还只在用户确认后入账，并保留对应任务、交付物和回读证据。
