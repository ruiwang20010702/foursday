...
任务语义不进入Control Store。Control Store继续只保存会话级暂停/接管、`ownerRevision/sendGeneration`和待消费控制事件；`foursday-task-ledger.mjs`另存当前任务合同的私有投影，权限600、最多1000项，不保存聊天原文、人员身份或凭据。Codex通过`foursday_update_task_contract`提交标题、目标、交付物、验收条件、生命周期与有界证据摘要；MCP从短期contextToken派生匿名`sourceSessionHash`、项目、owner revision和send generation，模型不能指定这些身份字段。旧generation不能覆盖新合同，`waiting_acceptance`至少包含一条verified证据，Agent输入枚举不含`accepted`。正式验收未来由稳定请求人/负责人反馈写入独立入口，绝不由生成答案的同一模型自证。
