...
宿主 Node 侧车只暴露完成当前动作所需的最小接口。DWS、gbrain OAuth、数据库密钥和 Git 凭据不进入项目终端环境。项目钉钉正本读取通过MCP专属`DWS_PATH/FOURSDAY_DWS_HOME`执行固定的`doc +fetch`命令；这两个变量写入`[mcp_servers.foursday.env]`，不进入Codex App Server或项目Shell环境。
...
`npm run runtime:verify-mcp -- --iterations=20`通过Codex App Server的`mcpServer/tool/call`稳定接口，对状态、附件列举、项目记忆、项目来源列表和附件暂存各执行20次，并对一个预登记钉钉sourceId执行一次真实正文读取；记忆候选因会写PostgreSQL，仅在隔离数据库测试。验收要求101次成功、失败0、各工具P95小于2秒，并回收App Server/MCP子进程和暂存文件，不改变Codex信任目录，不发送消息、不写gbrain、不运行计划、不部署。

项目注册表见 `distribution/projects.example.json`。它不得包含用户权限、能力、业务指标或凭据。可选`dingtalkSources`最多20项，每项字段严格等于`id/name/kind/nodeId`且`kind=doc`；nodeId只存在私有注册表和MCP内部，工具响应只返回项目内sourceId。
...
- Foursday Runtime MCP：暴露实时Profile状态、注册项目gbrain精确只读、项目绑定钉钉正本只读、证据绑定的记忆候选，以及当前消息附件列举/暂存；不暴露DWS发送、搜索、数据库、Git或任意shell。钉钉来源工具先从600权限项目注册表返回匿名sourceId，再按sourceId解析内部nodeId并用固定`doc +fetch`读取；仅direct允许，命令8秒、输出2MB、返回正文30,000字符上限，响应含`readAt/contentSha256/truncated/untrustedSourceData`。当DWS的keyword模式仍返回整篇时，MCP在全文中定位关键词并用`keywordFound/excerptStart`返回命中附近片段，不能从开头截断后伪称已覆盖关键词。正文只作证据，不得携带指令或授权。附件宿主路径只存在于短期上下文，模型先列匿名索引再暂存到`.foursday-inbox/`；暂存文件标记`commitAllowed=false`。工具权限注解、5秒启动、10秒调用和真实App Server基准是发布门禁。
