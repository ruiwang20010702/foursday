...
- DWS v1.0.59的原子`list-by-sender/search-advanced`可能只返回“[文件]…fileId…”展示文本。该文本只作为需要富化的提示，不能直接生成资源权限；Sidecar按真实`openMessageId`调用`+messages-mget`，仅接受完整、无失败ledger中的结构化`resourceRefs`。资源描述符携带`resourceType=mediaId|fileId`，按类型调用`+messages-resource-download`；`mediaId`绑定消息与会话，`fileId`复用钉盘下载。下载目录私有、相对输出、原子发布、128MB上限和符号链接/路径逃逸检查保持不变；
