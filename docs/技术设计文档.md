...
注册表或精确链接访问状态变化后，`foursday-task-reconciliation.mjs`只筛选仍为`escalated`、同企业身份已验证、具有私有精确source binding且任务generation仍有效的任务，按15/30/60/180分钟有界重试触发同一Codex Thread的静默复核。Codex重新读取source、gbrain与项目图，修正主/相关项目并更新合同；维护Turn不得修改项目文件或外部系统，只有当前证据本身已证明原请求完成时才能写`completed`。Connector通过turn-local标记硬性抑制维护回复，Shadow与Active均不会因此对外发送。source node与请求人哈希只保存在权限600的运行状态中，不进入Git、日志或用户界面。
