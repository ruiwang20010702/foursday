...
macOS用户还可以构建可选桌面伴侣。它复用已经安装的Codex pet素材，透明悬浮并根据空闲、工作、等待、验收和异常显示Foursday状态；悬停时挥手、点击时跳跃并展开同一个任务账本、拖动时朝对应方向奔跑，任务完成后短暂庆祝。互动动画不改变业务状态。它不修改Codex App，也不创建第二状态机：
