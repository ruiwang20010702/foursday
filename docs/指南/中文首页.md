...
当前源码候选版本为 `v0.9.0-rc.4`，最新 GitHub Release 仍为 `v0.8.0-rc.1`。源码候选、GitHub Release、技术部署和生产激活是相互独立的状态；真实发送仍需影子验收、显式激活和回读。
...
普通用户优先使用一个命令完成检测、项目接入、试用启动和真实Codex只读验证：
...
向导最多询问钉钉账号和允许工作的项目根目录两个问题。它始终停在“试用中，不会自动回复”，不会自动开启发送、gbrain写入、主动工作或计划任务。分步命令仅用于高级诊断与恢复。
...
macOS默认可视化入口是桌宠统一工作现场。它复用已经安装的Codex pet素材，透明悬浮并根据空闲、工作、等待、验收和异常显示Foursday状态；点击后先汇总“需要我／AI负责／最近完成”数量，再以项目优先的侧栏直接展开任务。同一项目在整个侧栏只出现一次，不用下拉切换，也不会把多条任务显示成多个项目。绑定优先匹配当前任务代次；短暂未路由的新任务显示“正在识别项目”，旧孤儿记录进入“未归档历史”。项目归属由Codex自动判断，不要求同事创建项目名或文件夹。“AI负责”只表示任务所有权仍在AI，不代表此刻有Turn运行。详情显示经过身份校验的联系人显示名与渠道、`Foursday · Codex`执行者、真实Thread位置、最近更新时间和脱敏活动；稳定身份ID仍不展示也不依赖显示名鉴权。没有任务合同时明确说明证据尚未开始统计。隔离Thread不会出现在主Codex侧边栏，界面只提供诊断编号复制，不展示无效深链。普通任务仍默认自主执行，面板不是审批中心；它不展示原始思维链、不修改Codex App，也不创建第二状态机。

Gateway启动时会安全增量接入Codex侧边栏中明确保存的本地项目，只增不删并排除整个主目录、系统目录、宽泛目录和符号链接。消息内文档链接产生的`shared_link`只是当前Turn的临时读取范围；Codex选定项目后，桌宠立即按真实项目分组，并说明下一Turn才切换执行目录。权限类请求先只读核验当前请求人的ACL角色：已具备权限直接完成，无法核验只显示“暂时无法核验”，真实授权变更仍需负责人决定。

工作现场右上角的“设置与系统诊断”是唯一高级状态入口。第一层只显示钉钉连接、Codex工作环境、消息同步、个人记忆、版本、证据和主动工作；展开技术详情后才显示Shadow/Active、发送开关、Checkpoint代次、Event Wake、reaction、固定绑定、可发现项目、精确提交和固定错误码。异常只给一个推荐动作，不展示聊天正文、身份ID、绝对路径、原始日志、命令、凭据或思维链：
...
桌宠会在需要时自动启动`127.0.0.1:9466`只读Control服务。桌宠不可用或非macOS环境下，才手动运行`npx --no-install foursday dashboard`打开隐藏的浏览器应急页；它读取同一Control Service，不保存第二套状态，也没有写接口。旧9465页面属于已移除的旧架构，不作为新版本操作入口或状态权威；迁移清理不得连带删除PostgreSQL、钥匙串或gbrain数据。
