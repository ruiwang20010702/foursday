...
可信联系人提出完整目标后，搜索、读取、Web、图片、项目MCP、Python、文件修改、测试、本地commit、Thread恢复/分叉和同项目子Agent在一次任务授权范围内自主完成，不做逐工具审批。业务口径、内容和验收询问任务请求人；账号、跨项目、生产、秘密、高权限个人MCP和不可恢复动作询问负责人。
...
| Foursday MCP | 最小可信工具 | direct/group/cron可只读当前注册项目的个人gbrain精确slug；私聊可创建证据绑定候选；私聊/群聊可按短期令牌列举并暂存当前附件；Cron拒绝附件与记忆写入 |
...
当前源码候选只允许已绑定的`thread/resume/fork`：持久绑定profile、platform、principal、conversation、project、规范CWD、Codex Thread、Hermes Session、permission version与`ownerRevision/sendGeneration`，并在每次恢复/分叉前重新校验；Fork登记采用跨进程锁且限于同一任务。Codex Thread正文不复制到Foursday第二数据库。
...
- Push、Merge、Release、生产、跨项目、个人高权限MCP、登录态Browser、Shell任意网络、秘密、付款、合同、人事和不可恢复动作询问负责人。
