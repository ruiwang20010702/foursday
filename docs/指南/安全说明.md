...
- 只有openDingTalkId时，显示名仅用于搜索当前组织候选；必须由相同openDingTalkId唯一反解staff userId，禁止用显示名或请求参数回填后自证。
- 当前组织身份链无法精确回读相同userId的消息不调用模型、不读取项目、不回复。
- 明确组织策略拒绝、身份错配和外部联系人立即失败关闭；网络/目录瞬时失败仅进入`600`权限的有界身份隔离队列，默认30分钟、8次、128条、单身份8条，验证前绝不创建Session或进入Codex。
- 隔离队列只记录必要消息信封，日志只写消息哈希和固定错误码；过期、容量拒绝和永久拒绝不输出正文。它不能用于重试发送，发送结果未知仍禁止自动重试。
- 启动历史回放必须由已注册Adapter显式请求；Sidecar transport ready不等于消息已被Agent接纳，禁止在Adapter注册前先消费并永久去重。
- DWS不使用Hermes pairing作为企业准入。Adapter只在当前消息已通过DWS企业/显式用户门禁后，对该条`SessionSource`设置`role_authorized=true`；标记不持久、不能跨消息或跨重启复用。Profile对任何未授权私聊静默`ignore`，不得发送配对码或暴露运维命令。

### 3.2 群聊

- 同时要求白名单会话和明确 `mentionedSelf=true`。
- 未 `@` 消息不能因为包含项目关键词而触发。
- Gateway只信任已经同时通过稳定用户、注册群和明确`@`三项检查的Adapter事件；pairing不能扩大群聊权限。
...
- 个人gbrain原文不因Foursday而修改；Foursday只生成当前Turn的安全投影。私钥、密码、Token、`confidential`页面和人物页敏感信息仍整页拒绝。
- 仅项目页可按Markdown内容块省略人物敏感信息；脱敏后必须二次扫描且保留实质正文。上下文显式标记省略块数量，不得把脱敏视图描述成完整原文。
...
同账号在相关会话的真实人工消息先冻结本地`sendGeneration`，旧generation立即失去发送资格；冻结发生在语义模型之前。AI标签、既有发送ID、同次自动正文、引用、转发、截图OCR或他人复述均不算本人授权。

随后使用无工具Codex分类回合在五个固定枚举中解释语义；模型没有控制工具，也不能直接修改revision。Connector验证枚举与置信度后执行：沟通接管只转移对外发言权，Codex后台分析可继续；任务纠正进入同一Thread并替换旧方向；任务接管停止Turn、Fork/子Agent和后续主动触发；恢复请求重新校验权限后续跑；无关消息不影响任务。仅保留少量紧急停止本地快速通道；模型超时、失败、低置信度或第三方会话歧义均保守按沟通接管处理。若人工回复与项目证据冲突，AI对外保持沉默并私下展示差异，不自动公开反驳或写入长期记忆。

全局暂停和恢复不得改写任务自身的`active|paused|taken_over`，只失效或恢复全局执行门禁；单任务暂停继续独立保留。任务接管只冻结当时的任务generation，不得永久封锁该联系人；后续消息由受限无工具Codex判断是否依赖旧任务，只有明确旧任务续接才保持静默。新任务通过精确revision重开，分类失败也不得永久吞掉消息；高风险出口仍按负责人门禁失败关闭。暂停、任务接管和新任务必须分开，不能把“负责人曾接管”伪装成联系人永久失能。
...
钉钉“好的”reaction只表示Foursday已承担当前消息，不能授权工具、扩大项目范围或证明任务完成。自动reaction沿用Hermes处理生命周期Hook，但DWS Personal必须额外绑定任务锚点、generation和平台回执。reaction写入失败不阻断任务；发送前仍执行人工介入与唯一回执门禁。

负责人reaction只有在DWS提供稳定operator openDingTalkId、目标消息、添加/取消动作和服务端时间后才可产生控制事件。展示名、截图、emoji计数、转发内容和无时间快照均不能自证负责人身份。负责人添加reaction命中活跃任务后先冻结旧generation，再确定性沟通接管；reaction本身就是回复，不调用Codex、不补发AI文本。负责人手动移除当前自动“好的”也视为接管，其他移除只审计且不恢复AI发言。AI自动添加/移除reaction必须通过独立幂等账本排除，避免同一个人钉钉账号自回流。

若负责人reaction比对应消息正文更早到达，只缓存稳定事件信封，不缓存聊天正文；消息锚点出现后按原服务端时间重放并再次核对负责人、会话和发送代次。缓存最多5分钟、128条，过期不执行。多段消息合并时责任锚点必须迁移到最后一条，旧Shadow/在途标签随generation清理。

接管完成后，任何晚到的处理开始Hook都只能得到`responsibility_claim_stale`，不得重新添加或保留“好的”；重启也会清理已接管会话的残留责任记录。

事后reaction快照只含显示名时不得据此认定负责人。Foursday从DWS消息Stream取得稳定会话与发送者OpenID，在正文读取前建立该会话的reaction监听；重启仅预热已经保存稳定OpenID的活跃会话。缺稳定身份时保持未接管，不使用姓名、截图或模糊搜索补权。

其他群成员reaction不能接管负责人所有权。若reaction身份读取失败或时序不明，系统不得按展示名猜测；可能与负责人介入冲突的旧回复应失败关闭。自动“好的”标签必须晚于上述reaction接管能力上线，不能先开放标签再补自回流保护。
