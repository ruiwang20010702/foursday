...
test("pet reads only the loopback task projection and reuses Codex pet assets", async () => {
  ...
  assert.doesNotMatch(source, /DragGesture\(/u);
  assert.match(source, /translation\.width < 0 \? \.runningLeft : \.runningRight/u);
  assert.match(source, /private var dragReleaseTask: Task<Void, Never>\?/u);
  assert.match(source, /private var dragSettleTask: Task<Void, Never>\?/u);
  assert.match(source, /private var isDragging = false/u);
  assert.match(source, /private var isSettlingAfterDrag = false/u);
  assert.match(source, /if isSettlingAfterDrag \{ return \.idle \}/u);
  assert.match(source, /private var hoverEffectsResumeAt = Date\.distantPast/u);
  assert.match(source, /watchForDragRelease\(\)/u);
  assert.match(source, /Task\.sleep\(for: \.milliseconds\(80\)\)/u);
  assert.match(source, /NSEvent\.pressedMouseButtons & 1 == 0/u);
  assert.match(source, /NSEvent\.pressedMouseButtons & 1 != 0/u);
  assert.match(source, /Task\.sleep\(for: \.milliseconds\(240\)\)/u);
  assert.match(source, /if interactionState == \.runningLeft \|\| interactionState == \.runningRight/u);
  assert.match(source, /guard isDragging else \{ return \}/u);
  assert.match(source, /hoverEffectsResumeAt = Date\(\)\.addingTimeInterval\(0\.8\)/u);
  assert.match(source, /Date\(\) >= hoverEffectsResumeAt/u);
  assert.match(source, /matching: \[\.leftMouseDown, \.leftMouseDragged, \.leftMouseUp\]/u);
  assert.match(source, /NSEvent\.removeMonitor\(mouseEventMonitor\)/u);
  assert.match(source, /private func handleMouseEvent\(_ event: NSEvent\)/u);
  assert.match(source, /event\.window === panel && petHitRect\.contains\(point\)/u);
  assert.match(source, /panel\.frame\.width - 132/u);
  assert.match(source, /hypot\(translation\.width, translation\.height\) < 4/u);
  assert.match(source, /petMouseDownLocation = nil/u);
  assert.match(source, /petDragActive = false/u);
  assert.doesNotMatch(source, /Thread\.sleep|usleep/u);
