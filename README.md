...
- a private semantic task ledger: Codex derives the goal, deliverables, acceptance criteria and evidence state from full context, while Foursday enforces identity, revision and lifecycle boundaries; verified reversible work can finish automatically, while only the requester or owner can mark business acceptance;
...
The current source candidate is `v0.9.0-rc.5`; the latest GitHub Release remains `v0.8.0-rc.1`. The candidate adds live Codex-project synchronization, immediate semantic task reassignment, silent stale-task reconciliation, privacy-safe requester ACL verification, version-aware companion recovery, guided setup and the unified macOS worksite. A source candidate, GitHub Release, technical deployment and production activation are separate states.
